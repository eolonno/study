...
class CMatrix
{
	...
	bool MatrixOnFile(char*);			//считать матрицу из файла
	bool WriteMatrixInFile(char*);		//записать матрицу в фаил
	double MaxElement();			  	// Максимальный элемент матрицы
	double MinElement();				// Минимальный элемент матрицы
	void ShowMatrix(char* before);		// Вывести матрицу
	void SetElement(int x, int y, int value); //заполнить ячейку x y
};
