...
{
	pFunc = pF;// Указатель на функцию f(x,y), описывающую поверхность
	SpaceRect.SetRectD(RS.left,RS.top,RS.right,RS.bottom);// область, на которую
	// опирается поверхность в мировой СК
	MatrF.clear();
	MatrView.clear();
	MatrWindow.clear();

	CreateMatrF(dx,dy);// матрица для x,y,z,1
	CreateMatrView();//  матрица для x,y,1 на  плоскость XY
	CreateMatrWindow();
	}
