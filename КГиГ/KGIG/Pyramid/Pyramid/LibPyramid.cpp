...

{
	...
	CRectD RectView;
	GetRect(ViewVert, RectView);		// получаем охватывающий прямоугольник 
	CMatrix MW = SpaceToWindow(RectView, RW);	// матрица для пересчёта в оконные координаты 

	CPoint MasVert[5];	// Массив оконных координат вершин
	CMatrix V(3);
	V(2) = 1;
	for (int i = 0; i<5; i++)	// Цикл по количеству вершин - вычисляем оконные координаты вершин	
	{
		V(0) = ViewVert(0, i); // x
		V(1) = ViewVert(1, i); // y
		// x-xL, y-yL, 1
	...
	for (int i = 0; i<4; i++)
	{
		...
		sm = CosV1V2(VN, ViewCart);	//вычисляет косинус угла между двумя плоскостями

		if (sm >= 0) //вычисляет косинус угла между двумя плоскостями
		{
	...
	VN = VectorMult(R1, R2);	//вектор нормали к внешней грани
	sm = CosV1V2(VN, ViewCart);//вычисляет косинус угла между двумя плоскостями
	if (sm >= 0) //вычисляет косинус угла между двумя плоскостями
	{
		CPen Pen(PS_SOLID, 2, RGB(sm*sm*red, sm*sm*green, sm*sm*blue));
