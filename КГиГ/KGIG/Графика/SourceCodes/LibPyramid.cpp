...
// Готовим массив оконных координат для рисования
	CPoint MasVert[8];	// Массив оконных координат вершин
	CMatrix V(3); //V0(3);
	V(2)=1;
	//V0(0)=RectView.left;	
...
// Рисуем
  ...
  CBrush* pOldBrush =dc.SelectObject(&Brus);
	...
	for(int i=0;i<4;i++)	
	{
		...
		CMatrix V2=R3-R2;			      // Вектор - ребро сверху
...
// Рисуем
  ...
  CPen* pOldPen =dc.SelectObject(&Pen);
	...
	//dc.Polygon(MasVert + 4, 4);	// Верхушка
	dc.MoveTo(MasVert[4]);
	dc.LineTo(MasVert[5]);
	dc.LineTo(MasVert[6]);
	dc.LineTo(MasVert[7]);
	...
	for(int i=0;i<4;i++)	
	{
		dc.MoveTo(MasVert[i+4]);	// Перо на вершину E
		dc.LineTo(MasVert[i]);
	}
...
// Рисует пирамиду С УДАЛЕНИЕМ невидимых ребер + заливка и освещение
// Самостоятельный пересчет координат из мировых в оконные (MM_TEXT)
// dc - ссылка на класс CDC MFC
// PView - координаты точки наблюдения в мировой сферической системе координат
// (r,fi(град.), q(град.))
...
// Рисуем
	...
	for(int i=0;i<4;i++)	
	{
		...
		R3=Vertices.GetCol(i+4,0,2);
		CMatrix V1=R2-R1;			      // Вектор - ребро в основании
		CMatrix V2=R3-R2;			      // Вектор - ребро в верхушке
		VN=VectorMult(V2,V1);		  	// Вектор ВНЕШНЕЙ(!) нормали к грани
