...
int RightLeaves(Tree*, bool isRight = false);
...
void main()
{
	...
	for (;;)
	{
		...
		cout << "8 -	подсчет кол-ва правых листьев\n";
		cout << "9 -	выход\n";
		cout << "$ ";
		cin >> choice;
		cout << "\n";
		...
		case 8: cout << "Найдено " << RightLeaves(Root) << " правых листьев" << endl; break;
		case 9:  exit(0);
		}
	}
}
...
int RightLeaves(Tree* Root, bool isRight)
{
	int counter = 0;
	if (Root->Right != NULL)
		counter += RightLeaves(Root->Right, true);
	if (Root->Left != NULL)
		counter += RightLeaves(Root->Left);
