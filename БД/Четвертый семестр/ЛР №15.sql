...
INSERT INTO WEATHER(TOWN,STARTDATE,FINISHDATE,TEMPERATURE)
	VALUES('Минск', '2017-01-01 00:00', '2017-01-01 23:23', -6),
			('Могилев', '2017-01-01 00:00', '2017-01-01 23:23', -7),
			('Витебск', '2017-01-01 00:00', '2017-01-01 23:23', -10),
			('Гродно', '2017-01-01 00:00', '2017-01-01 23:23', -5),
			('Брест', '2017-01-01 00:00', '2017-01-01 23:23', -1),
			('Гомель', '2017-01-01 00:00', '2017-01-01 23:23', -2)

GO
CREATE TRIGGER TR_WEATHER ON WEATHER AFTER INSERT, UPDATE
AS DECLARE @T VARCHAR(20), @SD DATETIME, @FD DATETIME, @TEMP INT, @IN VARCHAR(300)
DECLARE @INS INT = (SELECT COUNT(*) FROM INSERTED),
		@DEL INT = (SELECT COUNT(*) FROM DELETED)
...
BEGIN
	...
	SET @T = (SELECT TOWN FROM INSERTED)
	SET @SD = (SELECT STARTDATE FROM INSERTED)
	SET @FD = (SELECT FINISHDATE FROM INSERTED)
	SET @TEMP = (SELECT TEMPERATURE FROM INSERTED)


	DECLARE @T_V VARCHAR(20), @SD_V DATETIME, @FD_V DATETIME, @TEMP_V INT
	DECLARE @COUNTER INT = 0
	DECLARE INFORMATION CURSOR LOCAL STATIC
	FOR SELECT * FROM WEATHER

	OPEN INFORMATION
	FETCH INFORMATION INTO @T_V, @SD_V, @FD_V, @TEMP_V
	WHILE @@FETCH_STATUS = 0
		BEGIN
		IF @SD_V = @SD AND @FD_V = @FD
			BEGIN
	...
	IF @COUNTER > 1
		BEGIN
			RAISERROR('Повторяющееся значение', 10, 1);
			ROLLBACK;
		END;
	CLOSE INFORMATION


	SET @IN = @T + ' ' + CAST(@SD AS VARCHAR(20)) + ' ' + CAST(@FD AS VARCHAR(20)) + ' ' + CAST(@TEMP AS VARCHAR(5))
	INSERT INTO TR_AUDIT(STMT,TRNAME,CC)
		VALUES('INS', 'TR_WEATHER', @IN);
END;
ELSE IF @INS > 0 AND @DEL > 0
BEGIN 
	...
	DECLARE @T_V1 VARCHAR(20), @SD_V1 DATETIME, @FD_V1 DATETIME, @TEMP_V1 INT
	DECLARE @COUNTER1 INT = 0
	DECLARE INFORMATION CURSOR LOCAL STATIC
	FOR SELECT * FROM WEATHER

	OPEN INFORMATION
	FETCH INFORMATION INTO @T_V1, @SD_V1, @FD_V1, @TEMP_V1
	WHILE @@FETCH_STATUS = 0
		BEGIN
		IF @SD_V1 = @SD AND @FD_V1 = @FD
			BEGIN
	...
	SET @T = (SELECT TOWN FROM DELETED)
	SET @SD = (SELECT STARTDATE FROM DELETED)
	SET @FD = (SELECT FINISHDATE FROM DELETED)
	SET @TEMP = (SELECT TEMPERATURE FROM DELETED)
	SET @IN = @T + ' ' + CAST(@SD AS VARCHAR(20)) + ' ' + CAST(@FD AS VARCHAR(20)) + ' ' + CAST(@TEMP AS VARCHAR(5)) + ' ' + @IN
	INSERT INTO TR_AUDIT(STMT,TRNAME,CC)
		VALUES('UPD', 'TR_WEATHER', @IN);
END;
RETURN;
...
INSERT INTO WEATHER(TOWN,STARTDATE,FINISHDATE,TEMPERATURE)
	VALUES('Минск', '2017-01-01 00:00', '2017-01-01 23:24', -6);
